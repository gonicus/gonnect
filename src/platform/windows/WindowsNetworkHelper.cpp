    ...
    while (curAddress) {
        ...
        while (dnsServer) {
            ...
            if (dnsServer->Address.lpSockaddr->sa_family == AF_INET) {
                const auto* address = reinterpret_cast<const sockaddr_in*>(dnsServer->Address.lpSockaddr);
                inet_ntop(AF_INET, &address->sin_addr, buf, sizeof(buf));
                servers << buf;
            } else if (dnsServer->Address.lpSockaddr->sa_family == AF_INET6) {
                const auto *address = reinterpret_cast<const sockaddr_in6 *>(dnsServer->Address.lpSockaddr);
