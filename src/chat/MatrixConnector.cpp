...
IChatRoom *MatrixConnector::chatRoomById(const QString &roomId) const
{
...
void MatrixConnector::sendMessage(const QString &roomId, const QString &message)
{
    emit sendMessageRequested(roomId, message);
}
...
void MatrixConnector::resetUnreadCount(const QString &roomId)
{
    ...
    if (foundEvent) {
        emit resetUnreadCountRequested(roomId, foundEvent->eventId());
    }
}
