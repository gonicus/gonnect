...
class SqlcipherConan(ConanFile):
    ...
    def config_options(self):
        ...
        if is_msvc(self):
            # -fvisibility has no MSVC equivalent; a static .lib exports nothing anyway.
            self.options.rm_safe("hidden_visibility")

    def configure(self):
        if self.options.shared:
            self.options.rm_safe("fPIC")
            # A shared library compiled with hidden default visibility exports nothing.
            self.options.rm_safe("hidden_visibility")
    ...
    def _generate_msvc(self):
        ...
        if self.options.enable_fts5:
            # Makefile.msc has no configure equivalent; the amalgamation carries the
            # FTS5 sources and gates them on this define.
            opt_feature_flags.append("-DSQLITE_ENABLE_FTS5")
    ...
    def _generate_unix(self):
        ...
        if self.options.enable_fts5:
            tc.configure_args.append("--enable-fts5")
        if self.options.get_safe("hidden_visibility"):
